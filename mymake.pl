...
if ($mod_tarball =~/modules\.tar\.gz$/) {
    ...
    elsif ($opts{"with-hip"} && $mod_tarball) {
        $mod_tarball=~s/modules.*\.tar\.gz/modules-hip.tar.gz/;
    }
    if (!-e $mod_tarball) {
...
if ($opts{quick}) {
    ...
    if ($opts{"with-pmi"} or $opts{"with-pmi1"} or $opts{"with-pmi2"}) {
    ...
    if (-f "src/pmi/configure.ac" && !$opts{"3rd-party-pmi"}) {
        ...
        elsif ($opts{"with-pmi2"}) {
            my $L=$opts{"with-pmi2"};
            $I_list .= " -I$L/include";
            $L_list .= " -L$L/lib -lpmi2";
        }
    }
